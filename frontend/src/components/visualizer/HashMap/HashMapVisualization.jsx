...
export default function HashMapVisualization() {
  const { buckets, highlightedBuckets } = useHashMapStore();

  return (
    <div className="w-full space-y-4">
      <div className="text-center text-white/70 mb-4">
        Hash Function: <span className="text-purple-400 font-mono">hash(key) = (Σ key.charCodeAt(i) * 31^i) % 10</span>
      </div>
                ...
                <AnimatePresence>
                  {bucket.length === 0 ? (
                    ...
                    bucket.map((entry, entryIndex) => (
                      ...
                      >
                        <div className="px-4 py-2 bg-gradient-to-br from-green-500 to-emerald-600 rounded-lg border border-white/20 shadow-lg">
                          <div className="text-white font-semibold text-sm">
                            {entry.key}: <span className="text-white/90">{entry.value}</span>
                          </div>
                        </div>
      ...
      <div className="mt-6 grid grid-cols-3 gap-4">
        ...
        <div className="bg-white/5 border border-white/10 rounded-lg p-4 text-center">
          <div className="text-white/50 text-sm mb-1">Total Entries</div>
          <div className="text-white font-bold text-2xl">
